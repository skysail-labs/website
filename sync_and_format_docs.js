...
const docsDest = path.join(workspaceRoot, 'apps/docs/docs');

// 1. Recursive copy from portal/ to apps/docs/docs/ (excluding README.md)
...
console.log('Copying files from portal/ to apps/docs/docs/...');
...
function processYAMLQuotes(filePath) {
  ...
  const newLines = lines.map(line => {
    if (line.startsWith('description:')) {
      ...
      // If it contains a colon and is not already wrapped in double or single quotes
...
// 4. Strip "link" attribute from Docusaurus category json files for click-to-collapse
...
// Run through all rules in both directories
rules.forEach(rule => {
  const portalPath = path.join(portalSrc, rule.file);
  const docsPath = path.join(docsDest, rule.file);
  processDiagrams(portalPath);
  processDiagrams(docsPath);
});

// Run YAML quoting and diagrams on ALL markdown files
...
console.log('Stripping category index links for collapsible sidebar...');
