...
zero knowledge. It never sees the orders that produced the trade, and it does not
see the trade's **size or price** either: a settlement publishes note commitments
and the proof that value was conserved, not the amounts that moved. The enclave
sees the orders, but cannot move funds except by submitting a proof the chain
