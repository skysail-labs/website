...
| **3. Submit** | Client → Enclave | The signed order is sent over RA-TLS. The enclave routes its signed symbol to one market, then verifies the signature, boot session, strictly increasing per-key nonce, viewing point, note opening, recent proof root, and input proof. It never touches a Solana transaction. |
