...
sidebar_position: 1
title: Overview
description: Darknyx is a privacy-preserving spot darkpool on Solana - hidden orders matched inside an attested confidential VM, settled trustlessly with zero-knowledge proofs.
---

# Overview

:::info[TL;DR]
Darknyx is a **privacy-preserving spot darkpool on Solana**. You submit hidden
...
Unlike an off-chain matching desk, Darknyx does not ask you to trust the operator
...
No single component - not Solana, not the operator, not a network observer - sees enough to deanonymize your trading.
